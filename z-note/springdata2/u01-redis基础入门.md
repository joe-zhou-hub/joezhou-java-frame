...
**概念：** redis所有的key都是字符串类型，而value则可使用 `string/set/hash/list/zset` 五种数据类型，除string外的容器型数据结构必须遵守以下规则：
...
- `keys *`：返回所有key。
- `keys h*`：返回以 `h` 开头的所有key。
- `keys h[a-s]*`：返回以 `h` 开头且第二位是 `a-z` 中字母的所有key。
- `keys ph?`：`?` 表示一个字符占位。
- `dbsize`：返回key总数，轻量级命令。
- `exists name`：返回指定key是否存在，存在为1，不存在为0。
- `del name`：删除指定key，成功删除返回1，不存在返回0，同时删多个用空格分隔。
- `expire name 20`：设置指定key在20秒后过期。
- `ttl name`：返回指定key的过期时间，不存在为-2，永生为-1。
- `persist name`：删除指定key的过期时间，使其永生。
- `type key`：返回指定key的数据类型，不存在返回none。

## 3.2 字符串string

**概念：** string类型包括字符串，整数（自动转型，存值范围与long类型相同），二进制（如jpg图片或序列化对象）等，常用于缓存，计数器，分布式锁等场景：
- 存储特性：一个string类型的key最多能存储512MB的value：
    - 当string值小于1MB时，扩容都是加倍现有空间。
    - 当string值大于1MB时，每次扩容1MB，直到512MB。
- 常用命令：`nil` 表示无值，任何变量在没有被赋值之前的值都为 `nil`：
    - `set a 1`：永久存储 `a=1`，同名覆盖，无论key是否存在都成功，总是返回 `OK`：
    - `setnx a 1`：永久存储 `a=1`，同名覆盖，key不存在时才成功并返回1，否则返回0。
    - `set a 1 xx`：永久存储 `a=1`，同名覆盖，key存在时才成功并返回 `OK`，否则返回 `nil`。
    - `mset a 1 b 2`：批量永久存储 `a=1` 和 `b=2`，原子操作节省网络IO次数，但为重量级命令。
    - `get a`：获取 `a` 值，key不为string报错，key不存在返回 `nil`。
    - `mget a b`：批量获取 `a` 值和 `b` 值，原子操作节省网络IO次数，但为重量级命令。
    - `del a`：删除 `a`，返回 `(integer) 1`。
    - `incr a`：自增 `a` 值，返回自增后结果，key不存在设置 `a=1` 并返回1。
    - `decr a`：自减 `a` 值，返回自减后结果，key不存在设置 `a=-1` 并返回-1。
    - `incrby a 5`：将 `a` 值自增5并返回，key不存在设置 `a=5` 并返回5。
    - `decrby a 5`：将 `a` 值自减5并返回，key不存在设置 `a=-5` 并返回-5。
    - `incrbyfloat a 1.5`：将 `a` 自增1.5并返回字符串1.5，负数表示自减。
    - `setex a 8 5`：存储 `a=5`，并设置10秒后key过期，返回 `OK`。
    - `getset a 5`：将 `a` 值修改为 `5` 并返回原值，原子组合操作。
    - `append a 5`：将 `5` 追加到 `a` 值末尾。
