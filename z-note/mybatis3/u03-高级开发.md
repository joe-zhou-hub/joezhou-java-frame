...
**需求：** 找到所有名中含"王"的男用户，若姓名为null，则按性别精准查询，若性别为null，则按姓名模糊查询，
若姓名和性别均不为null，则按姓名模糊且按性别精准查询，若姓名和性别均为null，全查：
- `<where>`：WHERE语句不为空时，生成 `WHERE` 关键字并删除WHERE语句中第一个 `AND`。 
- `<trim prefix="WHERE" prefixOverrides="AND|OR">`：WHERE语句不为空时，生成 `WHERE` 关键字并删除WHERE语句中第一个 `AND` 或 `OR`。 
...
**需求：** id不为null时，若姓名不为null，按姓名模糊查询，若姓名为null，但性别不为null，则按性别精准查询，若姓名和性别都为null，则按照id精准查询：
- `<choose>`：结构根标签，用于SQL语句块内的多选一。
- `<when test='OGNL表达式'>`：用于分支条件判断，表达式建议用单引号，以免冲突表达式中的字符串。
- `<otherwise>`：用于设置所有条件均不满足时的处理。

**源码：** /mybatis3/
- res: `classpath:com/joezhou/mapper/UserMapper.xml`
- src: `c.j.mapper.UserMapper`
- tst: `c.j.dynamic.UserTest.findLikeNameAndGenderByChoose()`

# 5. 动态查询foreach

**需求：** 找到编号为3和4和6的用户：
- `<foreach>`：用于SQL语句的循环拼接：
...
- res: `classpath:com/joezhou/mapper/UserMapper.xml`
- src: `c.j.mapper.UserMapper`
- tst: `c.j.dynamic.UserTest.findWithArrayByForEach()`
- tst: `c.j.dynamic.UserTest.findWithListByForEach()`
- tst: `c.j.dynamic.UserTest.findWithMapByForEach()`

# 6. 动态修改set

**需求：** 在修改操作中，若某个字段设置为null，则不进行修改。
- `<set>`：生成 `SET` 关键字并删除SET语句中第一个 `,`。 
- `<trim prefix="SET" prefixOverrides=",">`：生成 `SET` 关键字并删除SET语句中第一个 `,`。
