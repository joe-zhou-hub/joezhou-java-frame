...
<select id="retrieveByNameAndGender" resultMap="studentMap">
    SELECT <include refid="cols"/> FROM `student` WHERE
        `stu_gender` = #{stuGender} AND
...
public void retrieveByNameAndGender() {
    ...
    List<Student> students = mapper.retrieveByNameAndGender(student);
...
**概念：**
- `<if>` 标签可以进行某些SQL语句是否拼接的判断。
- `<if>` 中使用 `test` 来进行条件判断，`test` 中使用的是OGNL表达式。
- OGNL表达式中，`"y"` 是String类型，而 `'y'` 是char类型，二者不等，而我们一般情况下比较的都是字符串，所以外层的符号尽量写成单引号。
...
- 如果姓名为null，按照性别精准查询。
- 如果性别为null，按照姓名模糊查询。
- 如果姓名和性别不为null，则按照姓名模糊并且按照性别精准查询。
- 如果姓名和性别都为null，全查。

## 1.2 Mapper配置

**概念：**
- `<where>` 标签可以自动生成 `WHERE`。
- `<where>` 会自动的删除第一个 `AND`。 
...
public void retrieveByNameAndGender() {
    ...
    student03.setStuName("王");
    student03.setStuGender("male");
    ...
    List<Student> students04 = mapper.retrieveByNameAndGender(student04);

    System.out.println(students01);
    System.out.println(students02);
    System.out.println(students03);
    System.out.println(students04);
}
```
