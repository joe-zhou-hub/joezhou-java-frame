...
**源码：** /mybatis3/
- res: `classpath:mapper/student-mapper.xml`
- tst: `com.joezhou.crud.StudentTest.insert()`

## 2.1 内部主键查询

**概念：** `<insert>` 中的 `<selectKey>` 可以替代 `useGeneratedKeys="true"` 配置：
- `order`：相对于 `<insert>` 语句的执行时机，可选 `BEFORE` 或 `AFTER`。
- `keyProperty="id"`：指定主键回注到哪个字段中。
...
**源码：** /mybatis3/
- res: `classpath:mapper/student-mapper.xml`
- tst: `com.joezhou.crud.StudentTest.insertWithSelectKey()`

## 2.2 非自增主键回注

**概念：** 如果数据库的主键采用的是UUID随机字符串，那么添加时只能使用 `<selectKey>` 进行回注：

**源码：** /mybatis3/
- res: `classpath:mybatis-teacher.xml`
- res: `classpath:mapper/teacher-mapper.xml`
- src: `c.j.pojo.Teacher`
- tst: `c.j.crud.TeacherTest.insert()`
