...
- SQL语句中，`#{xxx}` 是占位符，一般用于等号后：
    - 如果参数是实体类，则占位符中必须填写实体类中对应的属性名，可以连点。
    - 添加业务中，当主键为自增时：
        - 不指定id，则实体类中id属性值必须为null，否则报错。
        - 指定了id，则实体类中id属性值可以自己指定，若不指定，则仍为自增。
- 测试：
    - 获取一个具有事务保护的会话。
    - `sessoin.insert()`：添加数据：
        - param1：SQL语句块的id，需要使用命名空间为前缀。
        - param2：入参。
        - return：影响的条目数。
    - `sessoin.commit()/rollback()`：手动控制事务提交和回滚。
    - `session.close()`：关闭会话。
