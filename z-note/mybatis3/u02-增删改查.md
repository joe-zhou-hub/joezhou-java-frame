...
- 开发SQL语句：使用占位符 `#{}`。
- 测试：使用 `sessoin.update("命名空间.SQL语句块ID", 入参)` 修改数据。
...
- 开发SQL语句：使用占位符 `#{}`。
- 测试：使用 `sessoin.delete("命名空间.SQL语句块ID", 入参)` 删除数据。
...
**概念：** 接口动态代理方式需要额外配置一个接口，建议和SQL配置文件同名，并遵守如下对应关系：
...
public interface StudentMapper {
     ...
     * @return 所有符合条件的学生
     */
    List<Student> findLikeName(String stuName);
