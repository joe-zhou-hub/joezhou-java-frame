...
**概念：** 引入 `properties` 文件有两种方法：
- 在核心配置文件中，使用 `<properties resource="">` 引入属性文件，然后在数据源中使用OGNL表达式来获取属性文件中的值。
- 在创建会话工厂时，通过 `build()` 直接指定属性文件：
    - `new Properties()`：创建 `java.util.Properties` 对象。
    - `properties.load()`：将属性文件的字节流加载到 `properties` 中。
    - `factory.build(inputStream, properties)`

**源码：** /mybatis3/
- res: `classpath:mybatis-student.xml`
- tst: `c.j.start.StudentTest.useProperties()`
- tst: `c.j.start.StudentTest.usePropertiesInFactory()`
