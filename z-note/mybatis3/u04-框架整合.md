...
- `selectByExample()` 通过条件查询信息，需要三个步骤：
...
- 调用 `selectByExample()` 的时候，传递一个未设置任何条件的条件对象，则视为全查。
- 调用 `countByExample()` 可以按照条件查询符合条件的数据的条目数。
- 对条件对象调用 `setOrderByClause()` 可以对结果集进行排序。
- 对条件对象调用 `setDistinct()` 可以对结果集去重。
- 对条件对象调用 `setOrderByClause()` 可以对结果集进行排序。
