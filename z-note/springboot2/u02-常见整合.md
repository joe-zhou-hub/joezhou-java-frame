...
**概念：** WebSocket是HTML5提供的一种在单个TCP连接上进行全双工通讯的协议，B端和S端只需完成一次握手即可建立一条快速的，持久的双向数据通道，更节省服务器资源和带宽，且允许S端主动向B端推送数据：
- 添加依赖：`spring-boot-starter-websocket`
- 开发配置类：开发 `ServerEndpointExporter` 的 `<bean>`。
- 开发S端类：负责接收客户端的 `ws://` 连接请求：
    - 类上标记 `@ServerEndpoint()` 以指定端点地址，建议使用REST请求配合 `@PathParam` 取值。
    ...
    - `@OnOpen` 方法会在B端连入S端时执行，且只执行一次，可用形参直接获取请求参数和会话对象：
        - 使用 `webSocketServerMap` 存储用户ID以及当前用户的 `WebSocketServer` 对象。
        - 利用 `session.getAsyncRemote().sendText()`：向B端的 `onmessage()` 事件异步传递消息。
        - 利用 `session.getBasicRemote().sendText()`：向B端的 `onmessage()` 事件同步传递消息。
    - `@OnClose` 方法会在B端调用 `socket.close()` 时执行，可用形参直接获取请求参数和会话对象：
        - 从 `webSocketServerMap` 中 `remove()` 当前用户。
    - `@OnError` 方法在连接或通信错误时执行，可用形参直接获取 `Throwable` 对象和会话对象。
    - `@OnMessage` 方法可用形参直接收B端通过 `socket.send()` 方法传递过来的值。
    - `sendById()`：自定义方法，负责向指定ID的用户推送消息：
        - 通过ID找到这个用户私有的 `WebSocketServer`。
        - 通过 `WebSocketServer` 获取这个用户私有的 `session` 对象。
        - 利用 `session.getAsyncRemote().sendText()`：向B端的 `onmessage()` 事件异步传递消息。
    - `sendToAll()`：自定义方法，负责向所有已连接的用户推送消息：
        - 遍历 `webSocketServerMap`，得到通话中的所有用户的 `WebSocketServer`。
        - 通过 `WebSocketServer` 获取每个用户私有的 `session` 对象。
        - 循环向每一个客户端的 `onmessage()` 事件中发送消息。
- 开发动作类：
- 开发B端：编写两个html页面模拟两个用户。
    - 判断浏览器是否支持 `WebSocket`，一般主流浏览器都是支持的。
    - 新建 `new WebSocket()` 对象。
    - 设计 `WebSocket` 声明周期事件：
...
**源码：** /springboot2/
- src: `com.joezhou.springboot2.websocket.WebSocketConfig`
- src: `com.joezhou.springboot2.websocket.WebSocketServer`
- src: `com.joezhou.springboot2.websocket.WebSocketController`
- web: `classpath:templates/websocket/cli-a.html`
- web: `classpath:templates/websocket/cli-b.html`
- cli: 
    - 启动springboot项目（启动WebSocket服务）。
    - 访问：`cli-a.html`，登录用户用户A，查看用户A的浏览器控制台信息。
    - 访问：`cli-b.html`，登录用户用户B，查看用户B的浏览器控制台信息。
    - 访问：`/api/websocket/send-to-one?id=1&msg=hi!userA`，向用户A发送消息，查看用户A的浏览器控制台信息。
    - 访问：`/api/websocket/send-to-one?id=2&msg=hi!userB`，向用户B发送消息，查看用户B的浏览器控制台信息。
    - 访问：`./websocket/send-to-all.action?msg=hi every body`，向所有用户发送消息，分别查看用户01和02的浏览器控制台信息。
    - 用户A点击 `退出通话`，查看用户A的浏览器控制台信息。
    - 再次访问：`/api/websocket/send-to-one?id=1&msg=hi!userA`，向用户A发送消息，查看用户A的浏览器控制台信息。
    - 用户01点击：`恢复通话`，查看用户A的浏览器控制台信息。
    - 再次访问：`/api/websocket/send-to-one?id=1&msg=hi!userA`，向用户A发送消息，查看用户A的浏览器控制台信息。
