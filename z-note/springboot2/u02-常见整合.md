...
- 开发S端类：用于接收B端 `ws://` 开头的连接请求，每个B端都对应一个独有的S端实例：
    ...
    - 使用 `javax.websocket.Session` 类型属性存储B端和S端的长连接会话对象。
...
- web: `classpath:templates/websocket/cli-1.html`
- web: `classpath:templates/websocket/cli-2.html`
- cli: `websocket/cli-1.html`
- cli: `websocket/cli-2.html`
