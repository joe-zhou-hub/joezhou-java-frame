...
- 主配配置：自定义配置，代码会高亮不识别：
    - `joezhou.rocketmq.producer-group-name`：配置生产者自定义组名。
    - `joezhou.rocketmq.consumer-group-name`：配置消费者自定义组名。
    - `joezhou.rocketmq.name-srv-addr`：配置RMQ服务地址，如 `localhost:9876`。
...
- `@PostConstruct`：在对象构造器执行后调用，且只执行一次，内容为：
    - 创建一个生产者对象：同时将其放在指定的生产者组中，若没有这个生产者组会自动创建。
    - 指定RMQ服务地址，若集群则以分号分隔，如 `"192.168.1.1:9876;192.168.1.2:9876;"`;
...
- 在动作类中接收 `tag` 内容和 `msg` 具体消息，`topic` 写死，和 `MyConsumer` 中监听的 `topic` 名保持一致。
...
> 测试接口：rocketmq/send?topic=topic-a&tag=tag-a&msg=hi-rocketmq
