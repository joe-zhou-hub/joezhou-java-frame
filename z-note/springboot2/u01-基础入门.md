...
- 开发配置类：
    ...
    - 属性 `Boolean enabled` 与前缀组成 `joezhou.shield.enabled`。
...
- 开发自动装配方法：仿照 `RedisAutoConfiguration`：
    ...
    - `@ConditionalOnProperty`：仅在某个指定属性满足某些指定条件时才实例化该 `<bean>`：
        ...
        - `havingValue="true"`：指定属性的值为 `true/"true"` 时满足条件，缺省时则值不为 `false/"false"` 时都视为满足条件。
...
- 主配文件添加 `joezhou.shield.shield-words=a,b,c`
...
概念： 重新install我们的Starter组件之后，在测试项目中修改yml文件的配置，并测试，可以得出三种结果：

不配置 joe.shield.enabled：屏蔽词组件生效。
joe.shield.enabled = true：屏蔽词组件生效。
joe.shield.enabled = false：屏蔽词组件失效。
