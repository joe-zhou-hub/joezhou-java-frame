...
- 新建springboot项目 `shield-spring-boot-starter`，该start组件用于屏蔽敏感词：
    ...
    - 创建时需添加 `Spring Configuration Processor` 模块，建议添加 `Lombok` 模块。
- 开发功能类，封装一个方法负责完成屏蔽敏感词的功能，如将 `a/b/c` 替换成 `*`。
- 开发配置类：
    - `@Configuration`：标识该类为配置类。
    - `@ConfigurationProperties(prefix = "joezhou.shield")`：设置属性前缀。
- 开发自动装配类：仿照 `RedisAutoConfiguration` 得知，一个自动配置类的注解至少有三个：
    - `@Configuration`：标识该类为配置类。
    - `@ConditionalOnClass(RedisOperations.class)`：当拥有 `ShieldOperations` 功能类时生效。
    - `@EnableConfigurationProperties(ShieldProperties.class)`：读取配置文件
- 开发自动装配方法：仿照 `RedisAutoConfiguration` 得知，一个自动配置类中需要配置功能类对应的 `@Bean`：
    - `@Bean`：方法名对应 `id`，返回值对应 `class`。
    - `@ConditionalOnMissingBean`：类条件注解，表示当前容器中没有这个 `<bean>` 的时候才会实例化这个 `<bean>`：
        - 如果指定了 `name` 表示spring容器会通过byName的方式实例化这个 `<bean>`。
        - 如果没指定 `name` 表示spring容器会通过byClass的方式实例化这个 `<bean>`。
    - 在方法中读取属性文件的前缀的内容，并且通过 `setShieldWords()` 传递给 `shieldService` 中的 `shieldWords` 属性，即如果自动装配能够成功的话，这个 `shieldService` 中的 `shieldWords` 属性就已经获得了属性文件中，以 `joezhou.shield` 为前缀的属性的值了。 
- 配置spring.factories文件：`AutoConfigurationImportSelector` 最终会选择一种方式去导入每个starter组件中的 `META-INF/spring.factories` 文件中指定的自动装配类：
    - 文件中的 `key` 是 `EnableAutoConfiguration` 的类全名。
    - 文件中的 `value` 是我们的自动装配类的类全名。
- 将start项目install到maven仓库中：
    - 在 `pom.xml` 文件中的 `spring-boot-maven-plugin` 插件中添加 `<skip>` 配置，否则install出来的jar包结构中，会多出一层 `BOOT-INF` 目录，导致jar包中的类无法使用。

> 启动类注解自动导入了 `AutoConfigurationImportSelector.class`，跟踪 `getCandidateConfigurations() - loadFactoryNames() - loadSpringFactories()`，观察到该选择器最终会选择一种方式去导入每个starter组件中的 `META-INF/spring.factories` 文件。
...
> 点击install，将项目打包在maven仓库中。

# 8. 使用starter组件

## 8.1 新建测试项目

**概念：** 新建一个Maven项目，用于测试刚才写的starter组件是否能够完成自动装配。
- 创建项目的时候引入 `web` 组件即可。
- 在pom.xml文件中引入我们的 `shield-spring-boot-starter` 组件。
...
**概念：** 我们需要在项目的yml文件中配置过滤词汇，前缀使用我们 `ShieldProperties` 中设定的 `joe.shield`。
...
**概念：** 动作类中，注入 `ShieldService` 并且写一个接口方法完成测试。

**源码：** ShieldController.java
```java
/**
 * @author JoeZhou
 */
...
> 测试路径：http://127.0.0.1:8080/shield.action?word=ccaaccbbcc
