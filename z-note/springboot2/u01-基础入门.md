...
- res: `pom.xml`
- src: `c.j.s.App`
- src: `c.j.s.start.StartController`
...
- 添加依赖：`spring-boot-starter-tomcat`，作用于必须为 `provided`。
- 启动类继承 `SpringBootServletInitializer` 并重写 `configure()`：
    - `builder.sources(入口类.class)`。
- 将项目打成jar/war包：`mvn install`，并拷贝到 `%TOMCAT_HOME%\webapps` 中，建议jar/war包重命名。
...
- src: `c.j.s.restTemplate.RestTemplateController`
...
- src: `c.j.s.shield.ShieldController`
- tst: `c.j.s.shield.ShieldTest`