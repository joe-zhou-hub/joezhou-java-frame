...
**概念：** 响应式开发的业务类中：
- 原本返回的 `User` 类型替换为 `Mono<User>`，则数据将以异步不阻塞的方式返回给调用方。
    - `Mono.just(user)`：得到一个对应的 `Mono<User>` 对象，参数为null或者空会报错。
    - `Mono.justOrEmpty(user)`：得到一个对应的 `Mono<User>` 对象，参数为null或者空，返回一个MonoEmpty对象。
- 原本返回的 `List<User>` 类型替换为 `Flux<User>`，则数据将以异步不阻塞的方式返回给调用方。
    - `Flux.fromIterable(users)`：将一个可迭代对象转换为 `Flux<User>` 对象。
    - `Flux.fromArray(users)`：将一个数组转换为 `Flux<User>` 对象。
...
public interface UserService {
     ...
     * @return 满足条件的唯一数据
     */
    Mono<User> retrieveById(Integer id);
     ...
     * @return 被删除的数据
     */
    Mono<User> deleteById(Integer id);
}
```
...
public class UserServiceImpl implements UserService {
    ...
    static {
        users = new ArrayList<>();
        users.add(new User(1, "name01"));
        users.add(new User(2, "name02"));
        users.add(new User(3, "name03"));
    }

    @Override
    public Mono<User> retrieveById(Integer id) {
        User user = null;
        for (User e : UserServiceImpl.users) {
    ...
    @Override
    public Mono<User> deleteById(Integer id) {
        User user = null;
        for (User e : UserServiceImpl.users) {
            if (e.getId().equals(id)) {
                user = e;
...
public class MyUserController {
    ...
    @RequestMapping("/retrieve-by-id.action")
    public Mono<User> getUser(Integer id) {
        return userService.retrieveById(id);
    }

    @RequestMapping("/retrieve-all.action")
    public Flux<User> retrieveAll() {
        return userService.retrieveAll();
    }

    @RequestMapping("/delete-by-id.action")
...
> 测试接口：
> <br/> ./api/user/retrieve-all.action
> <br/> ./api/user/retrieve-by-id.action?id=1
> <br/> ./api/user/delete-by-id.action?id=1
