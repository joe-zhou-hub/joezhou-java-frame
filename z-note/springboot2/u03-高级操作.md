...
- payload：token负载，部分用户信息以避免多次查库，少加敏感信息：
    - iss: token发行人，可由 `jwtBuilder.setIssuer()` 设置。
    - iat: token发行时间，可由 `jwtBuilder.setIssuedAt()` 设置。
    - sub: token主题，可由 `jwtBuilder.setSubject()` 设置。
    - aud: token接收方，可由 `jwtBuilder.setAudience()` 设置。
    - exp: token过期时间戳，可由 `jwtBuilder.setExpiration()` 设置。
    - nbf: token生效时间戳，可由 `jwtBuilder.setNotBefore()` 设置。
    - jti: token的id，可由 `jwtBuilder.setId()` 设置。
...
**流程：** 整合到springboot：
- 配置pom依赖：`java-jwt`
- 开发注解 `c.j.s.jwt.PassToken`：标记此注解的类或方法可以跳过token验证。
- 开发注解 `c.j.s.jwt.LoginAuth`：标记此注解的类或方法必须先做登录验证。
- 开发业务类 `c.j.s.jwt.UserService/UserServiceImp`：开发 `selectById()`。
- 开发动作类 `c.j.s.jwt.UserController`：开发 `selectById()`。
- 开发拦截器 `c.j.s.jwt.AuthInterceptor`：获取并验证token
- 开发拦截器配置类：`c.j.s.jwt.InterceptorConfig`：实现 `WebMvcConfigurer`。
- 测试：不加注解的话默认不验证，登录接口一般是不验证的。在getMessage()中我加上了登录注解，说明该接口必须登录获取token后，在请求头中加上token并通过验证才可以访问：
- psm: `api/user/login-auth`，不添加token，登录失败，控制台抛出异常。
- psm：`api/user/login` 正常登录，该方法没有@LoginAuto，直接放行，登录成功，返回token
- psm：`api/user/login-auth`，不添加token，同时请求头加如登录成功后的那个token，通过验证。

- 从 http 请求头中取出 token，
- 判断是否映射到方法
- 检查是否有 `@SkipToken`，有则跳过认证
