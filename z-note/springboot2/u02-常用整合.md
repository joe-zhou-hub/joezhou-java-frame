...
**概念：** [actuator](https://docs.spring.io/spring-boot/docs/2.4.2/reference/html/production-ready-features.html#production-ready) 意为能够移动和控制某些事物的驱动器，微小的改动也可以让它产生巨大的反应，是springboot的一个附加特性，帮助你监视和管理上线的项目，可以使用HTTP端点来监控和进行审计，检查运行状况，指标收集等操作，一旦项目出状况能够第一时间反馈，如发短信或重启尝试恢复等，以保持项目的高可用：
- 配置pom依赖：`spring-boot-starter-web/spring-boot-starter-actuator`。
- cli测试：
    - `/actuator`：查看actuator对外暴露的所有接口，默认为 `health` 和 `info`。
    - `/actuator/health`：查看项目健康信息，上线时返回 `status=UP`。
    - `/actuator/info`：查看任意的项目信息，默认为空，需要自行配置。
- 主配添加：
    - `management.server.port`：配置actuator服务端口号，缺省默认8080。
    - `management.endpoints.enabled-by-default`：配置默认端点是否可用，默认true，仅shutdown不可用。
    - `management.endpoint.shutdown.enabled`：配置shutdown端点是否可用，默认false。
...
- 主配：
    ...
    - `management.endpoint.health.cache.time-to-live`：health端点的缓存存活时间，默认0，即不缓存。
- 健康检查器：health自带很多健康检查器，但前提是需要配置对应技术。
- 开发健康检查器类 `c.j.s.actuator.MyDiskSpaceHealthIndicator`：
