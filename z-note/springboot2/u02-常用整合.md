...
- psm测试：
    - `/actuator`：查看actuator对外暴露的所有接口，默认 `health` 和 `info`。
    - `/actuator/health`：查看项目健康信息，`status=UP` 表示项目健康。
    - `/actuator/info`：查看项目配置信息，默认为空，需要自行配置。
- 主配添加：以下配置选配：
    - `management.server.port`：配置actuator服务端口，默认8080。
    - `management.endpoints.enabled-by-default`：配置是否启用shutdown外的端点，默认true。
    - `management.endpoint.shutdown.enabled`：配置是否启用shutdown端点，默认false。
    - `management.endpoints.web.exposure.include`：启用端点列表，逗号分隔，`"*"` 表示全部启用：
        - `z-res/actuator常用端点.md`。
    - `management.endpoints.web.exposure.exclude`：禁用端点列表，逗号分隔，`"*"` 表示全部禁用。
...
- 开发自定义健康检查器类 `c.j.s.actuator.MyDiskSpaceHealthIndicator`，标记 `@Component`。
...
- info端点配置：默认展示主配中的 `info.xxx` 配置项内容。
- metrics端点配置：用于展示运行状态如jvm内存信息，tomcat信息，线程信息，垃圾回收信息等：
    - metrics端点默认禁用，需要手动开启。
