...
- 开发动作类 `c.j.s.controller.UserController`：
    ...
    - `SecurityContextHolder.getContext()`：获取security上下文环境。
    - `context.getAuthentication()`：获取security认证信息，该信息在登陆成功后可以随时随地获取。
    - `authentication.getAuthorities()`：获取所有的角色和权限。
    - `authentication.getName()`：获取用户名。
- 在 `configure()` 中的 `formLogin()` 后额外配置，所有URL都建议使用动作方法的路由：
    - `loginPage(URL)`：自定义登录页面路由。
    - `loginProcessingUrl(URL)`：设置自定义登录表单的 `action` 值。
    - `successForwardUrl()`：设置登录成功页面的路由，缺省跳转至上次的请求路径。
    - `failureForwardUrl()`：设置登录失败页面的路由，缺省跳转至 `loginPage()` 指定的路由。
...
- 开发登录页面 `templates\login.html`：
    ...
    - 设计表单，`action` 指向 `api/user/login`，该动作方法由security创建和调用。
...
- 开发登录成功页面 `templates\main.html`：
    ...
    - `sec:authentication="principal"`：获取认证信息中的主要信息，如密码，过期时间等。