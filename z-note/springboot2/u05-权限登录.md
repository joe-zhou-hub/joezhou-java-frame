...
- 在动作类 `c.j.s.controller.UserController` 中添加一个对应注销请求的路由方法。
- 在配置类的 `.formLogin()` 后添加一些注销相关的配置：
...
- 测试：登录再注销，跟踪 `F12/Application/Cookie/JSESSIONID` 的情况并测试注销后是否仍可以访问项目资源。


# 7. 重写登录业务

**概念：** 若希望登陆成功或失败时返回JSON数据而不跳页，则需要自定义登录成功或失败的处理器来覆盖掉security默认的登陆操作：
- 设计一个登陆成功后的处理器 MyLoginSuccessHandler：
    - 继承 SavedRequestAwareAuthenticationSuccessHandler，重写 onAuthenticationSuccess()。
    - 如果什么都不做的话，直接调用父类方法即可：
        - super.onAuthenticationSuccess(request, response, authentication);
    - 如果需要返回JSON则可以使用 response 对象进行回写数据。
    - 如果仍然需要跳页面，则可以利用 DefaultRedirectStrategy 对象进行重定向：
        - new DefaultRedirectStrategy().sendRedirect(request, response, "/main.html");
- 设计一个登陆失败后的处理器 MyLoginFailureHandler：
    - 继承 SimpleUrlAuthenticationFailureHandler，重写 onAuthenticationFailure()。
    - 如果什么都不做的话，直接调用父类方法即可：
...
- 在配置类中注入这两个处理器。
- 找到配置类中的 configure() 方法中的 successForwardUrl() 的位置，将其替换成处理器的绑定配置。
    
源码： MyLoginSuccessHandler.java
```java
/**
 * @author JoeZhou
 */
...
源码： SecurityConfig.java 中的 configure() 中修改
```java
.formLogin()
.loginPage("/api/login/login-page-routing.action")
.loginProcessingUrl("/api/login/login.action")
