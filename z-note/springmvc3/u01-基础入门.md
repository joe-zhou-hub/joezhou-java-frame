...
- 在 `web.xml` 中配置springmvc的前端控制器 `o.s.w.s.DispatcherServlet`：
    - 前端控制器初始化时会默认加载 `/WEB-INF/配对名-servlet.xml` 文件，建议通过 `<init-param>` 的 `contextConfigLocation` 来修改springmvc主配文件的位置。
    - `<url-pattern>` 用于设置前端控制器的拦截规则：
        - `/`：表示拦截一切，但是不拦截非静态资源。
        - `/*`：永远不要这样写。
        - `/user/insert.action`：表示只拦截 `/user/insert.action` 请求。
        - `*.action`：表示只拦截以 `.action` 为后缀的请求，注意前面不要加 `/`。
- 配置springmvc主配文件：配置三大组件和处理器Handler扫描：
    - `<mvc:annotation-driven />`：MVC注解驱动负责配置处理器映射器和处理器适配器这两个组件。
    - 配置视图解析器：建议配置前后缀，当然也可以不配置。
    - 视图解析器使用springmvc框架默认的 `o.s.w.s.v.InternalResourceViewResolver` 这个类。
    - 视图解析器的前后缀配置只作用于响应路径，不作用于请求路径。
- 开发处理器类：对类添加 `@Controller` 注解，并在springmvc主配文件中扫描：
    - `@RequestMapping`：用于设置方法的路由，一个项目中的路由不要重复。
    - 路由可以省略 `.action` 后缀，但请求URL中不可以省略。
    - 路由可以省略两端的 `/`。
    - 动作类的方法名可以随意一点，对于springmvc来讲不是很重要。
    - 如果方法的返回值是String类型的，则return后面可以直接写返回路径，称为逻辑视图名，配合视图解析器中配置的前后缀组成物理视图名。
