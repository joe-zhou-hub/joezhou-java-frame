...
**概念：** 
- SpringMVC支持在处理器适配器上自定义一个转换器来对参数类型进行转换。
- 一般我们会在SpringMVC的核心配置文件中，使用 `<mvc:annotation-driven/>` 来加载处理器适配器，于是我们可以在这个标签上进行转换器的配置。

# 2. 自定义转换器类

**概念：** 
- 我们自定义的转换器类需要实现 `org.springframework.core.convert.converter.Converter` 接口，重写 `convert` 方法
- 转换器类很智能，它只有发现你的接收端是使用 `Date` 类型接收的时候才会走转换器，如果不是用 `Date` 类型接收则不走转换器，所以这个转换器对 `String` 类型的 `name` 是无作用的。
...
**概念：** 将自己写的一个Converter类注入到 `FormattingConversionServiceFactoryBean` 类中的 `converters` 属性中（它是一个Set）。

**配置：** springmvc.xml
```xml
<!--在配置处理器映射器和处理器适配器的同时，指定转换器列表-->
<mvc:annotation-driven conversion-service="converterList">

<!--转换器配置，可以同时配置多个转换器-->
<bean id="converterList" class="org.springframework.format.support.FormattingConversionServiceFactoryBean">
    <property name="converters">
        <set>
            <bean class="com.joe.converter.MyDateConverter"/>
        </set>
    </property>
</bean>
```
...
public class DateController {
    ...
    @ResponseBody
    @RequestMapping("format.action")
    public String format(Date birth, String name) {
...
> 接口测试：date/format.action?birth=2001-01-01&name=zhaosi

> 接口测试：date/format.action?birth=2009/09/09&name=zhaosi
