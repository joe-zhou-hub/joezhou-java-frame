...
- `HttpStatus.CREATED`：http状态码201表示请求已成功，且有一个新资源被响应回客户端。
...
**概念：** 
- 对于SpringMVC来说，拦截器主要的工作对象就是用户的请求和响应，拦截下来之后，我们可以在拦截的各个阶段进行处理。
- 我们自己编写的拦截器类需要实现 `HandlerInterceptor`接口，并重写其中的三个方法：
    - `preHandle()`：拦截请求，可以在该方法中放入一些初始化的操作，比如权限验证，日志管理等：
        - 方法返回 `true`：放行。
        - 方法返回 `false`：程序终止，请求结束。
    - `postHandle()`：拦截响应，该方法可以对Controller处理之后ModelAndView进行操作。
    - `afterCompletion()`：渲染视图之后的方法，一般用于释放资源。
    
> 渲染就是将jsp中的一些变量程序（<%=...%>等）、css样式、js效果等都执行完毕并组合在一起的过程，最终结果是一张展示出去的静态页面。

# 2. 拦截器类

**源码：** MyInterceptor.java
```java
/**
 * @author JoeZhou
 */
public class MyInterceptor implements HandlerInterceptor {
    ...
    public void postHandle(HttpServletRequest req, HttpServletResponse resp,
        ...
        System.out.println("拦截响应：调用方法后，渲染视图前...");
    }
...
**概念：** 上面的拦截是对所有请求进行来接，如果你想指定拦截请求，该如下编写：
- 利用 `<mvc:mapping>` 可以指定拦截的请求。
- 利用 `<mvc:exclude-mapping>` 可以指定不拦截的请求。
- 二者同时存在时，取交集
...
<mvc:interceptors>  
    <mvc:interceptor>  
        <!--只拦截/interceptor/inter1.action请求，其他请求不触发拦截器-->
        <mvc:mapping path="/interceptor/inter1.action"/>
        <!--注册你的拦截器类-->
        <bean class="com.joe.interceptor.MyInterceptor"/>
    </mvc:interceptor>  
</mvc:interceptors> 
...
> 测试接口：/interceptor/inter1.action

# 5. 定义多个拦截器

**概念：** 
- SpringMVC中对于一个请求可以添加多个拦截器，而这个拦截器集合中会链式调用这些拦截器，比如你设计了拦截器1和拦截器2，则执行的顺序如下：
    - 拦截器1的 `preHandle()`
    - 拦截器2的 `preHandle()`
    - 业务方法 
    - 拦截器2的 `postHandle()`
    - 拦截器1的 `postHandle()`
    - 拦截器2的 `afterCompletion()`
- 注意，渲染只会执行一次，二者只执行最后一个拦截器的 `afterCompletion()`。
