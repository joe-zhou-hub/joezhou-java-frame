...
**概念：** `HandlerExceptionResolver` 是spring异常处理的顶级接口，其每个实现类都是一种异常处理方式：
- `ExceptionHandlerExceptionResolver`：使用注解方式捕获和处理异常：
    - 开发异常处理方法并添加 `@ExceptionHandler` 并使用 `value` 指定捕获的异常类，缺省捕获任意。
    - 异常处理方法只能包含一个异常类型参数，不能包含其他参数。
    - 异常处理方法可以移动到 `@ControllerAdvice` 标记的类中以复用，该类必须能被spring扫描到。
- `ResponseStatusExcpetionResovler`：使用 `@ResponseStatus` 来自定义异常页面。
...
> 接口测试：exception/exceptionHandler.action?num=0

# 3. 异常处理类

**概念：** 使用 `@ExceptionHandler` 修饰的异常处理方法只能作用于本类，如果想让工程中所有的类都能被它处理，则需要单独定义一个异常处理类：
- 该类需要添加 `@ControllerAdvice`。
- 该类需要添加 `@Component` 以保证可以被spring扫描到。

**源码：** ExceptionUtil.java
```java
/**
 * @author Joezhou
 */
...
**配置：** springmvc.xml
```xml
<context:component-scan base-package="com.joe.tools" />
```

> 接口测试：exception/exceptionHandler.action?num=0

# 1. ResponseStatusExcpetionResovler

**概念：** 
- `ResponseStatusExcpetionResovler` 是一种比较常用的实现类，主要依靠 `@ResponseStatus` 来自定义异常页面。

# 2. 异常处理方法

**概念：** 在某个动作类中单独写一个方法，它专门负责自定义异常页面，这个方法需要添加 `@ResponseStatus`：
- `value`：异常状态码。
- `reason`：提示信息。

**源码：** ExceptionController.java 中添加
```java
@RequestMapping("responseStatus.action")
public String responseStatus(Integer num) {
    int errNum = 2;
    if (num == errNum) {
        return "forward:responseStatusTool.action";
    }
    return "success";
}

@RequestMapping("responseStatusTool.action")
@ResponseStatus(value = HttpStatus.NOT_FOUND, reason = "页面走丢了！")
public String responseStatusTool() {
    return "forward:view/error.jsp";
}
```

> 测试接口：exception/responseStatus.action?num=2

# 3. 异常处理类

**概念：** 使用 `@ResponseStatus` 修饰的异常处理方法只能作用于本类，如果想让工程中所有的类都能被它处理，则需要单独定义一个异常处理类：
- 该类需要添加 `@ResponseStatus`。
- 该类需要添加 `@Component` 以保证可以被spring扫描到。
- 该类需要继承 `Exception`，因为一会我们要在业务方法中，直接 `throw new` 你的自定义异常。

**源码：** MyNotFoundException.java
```java
/**
 * @author JoeZhou
 */
...
```java
@ResponseBody
@RequestMapping("myNotFoundException.action")
public String myNotFoundException(Integer num) 
...
> 测试接口：exception/myNotFoundException.action?num=2

# 1. SimpleMappingExceptionResolver

**概念：** 
- `HandlerExceptionResolver` 是SpringMVC提供的异常处理的顶级接口，它的每一个实现类，就是一种处理异常的方式。
- `SimpleMappingExceptionResolver` 实现类通过XML配置的方式来处理异常。
...
public String simpleMappingExceptionResolver(Integer num) {
    if (num > 0) {
        String str = null;
...
**概念：** 
- `exceptionAttribute`：指定了异常信息存储的变量名，这个变量自动存入request域。
- 如果不指定 `exceptionAttribute`，则异常信息会被存入一个名字为 `exception` 的变量中，同样自动存入request域。
- `exceptionMappings`：指定了捕获的异常类型列表，中间的内容是跳转的页面，这个值会补充视图解析器中的前后缀。
...
> 测试接口：exception/simpleMappingExceptionResolver.action?num=1