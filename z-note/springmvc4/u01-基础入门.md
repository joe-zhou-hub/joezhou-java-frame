...
- 键值对简单参数：对方法参数使用 `@RequestParam` 可以获取指定key的键值对请求参数：
    ...
    - 允许直接使用实体类统一接收参数，要求实体类属性和请求参数同名。
    - 允许直接使用数组类型如 `Integer[]` 批量接收同名请求参数。
    - 若使用VO中的实体类接收参数，需要修改请求参数key值格式为 `VO中属性名.key`。
    - 若使用VO中的List接收参数，需要修改请求参数key值格式为 `VO中List属性名[下标].key`。
