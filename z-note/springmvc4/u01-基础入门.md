...
- 原生接值：动作方法形参中可直接使用原生请求对象，需要servlet依赖。
...
**概念：** 
- 原生存值：动作方法形参中可直接使用原生请求和会话对象存值，需要servlet依赖。
...
**概念：** 动作方法的转页方式取决于方法的返回值：
- 对视图解析器的 `<bean>` 注入前后缀属性可作用于某些响应路径：
    - `<property name="prefix" value="/view/"/>`：配置响应路径前缀。
    - `<property name="suffix" value=".html"/>`：配置响应路径后缀。
- `void` 返回值：需配合原生请求/响应进行请求转发/重定向/IO回写，不拼接前后缀。
- `String` 返回值：使用 `return` 直接返回请求转发路径，拼接前后缀：
    - `forward:`：请求转发到页面或动作类，不拼接前后缀，支持查询串。
    - `redirect:`：重定向到页面或动作类，不拼接前后缀，不支持查询串。
    - 若使用 `ModelAndView` 返回，可利用 `setViewName()` 设置请求转发路径，其余同上。

**源码：** /springmvc4/
- src: `c.j.controller.ForwardController`
- psm: `/api/forward/request-forward`
- psm: `/api/forward/response-redirect`
- psm: `/api/forward/response-writer`
