...
- 在web.xml中配置前端控制器类 `o.s.w.s.DispatcherServlet`：
    - 使用 `<init-param>` 配置前端控制器类启动加载项：
        - 注入 `contextConfigLocation` 指定spring主配文件位置。
...
- `@RequestMapping` 可以单独标记方法或同时标记类和方法，此时类路由成为方法路由前缀。
- `@RequestMapping` 可以对一个方法设置多组路由。
- `@RequestMapping` 可以设置通配符路由，
    - `?`：匹配单层路径中的一个字符，如 `/?/one`。
    - `*`：匹配单层路径中的任意字符，如 `/*/two`。
    - `**`：匹配多层路径，如 `/**/three`。
...
- psm: `{{tomcat}}/springmvc3/api/request-mapping/test1`
- psm: `{{tomcat}}/springmvc3/api/request-mapping/test2`
- psm: `{{tomcat}}/springmvc3/api/request-mapping/test3`
- psm: `{{tomcat}}/springmvc3/api/request-mapping/a/one`
- psm: `{{tomcat}}/springmvc3/api/request-mapping/abc/two`
- psm: `{{tomcat}}/springmvc3/api/request-mapping/a/b/c/three`
