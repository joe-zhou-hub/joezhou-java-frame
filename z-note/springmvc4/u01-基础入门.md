...
**概念：** 
- 接收请求头参数：对动作方法参数使用 `@RequestHeader` 注解可以获取请求头信息。
- 接收Cookie参数：对动作方法参数使用 `@CookieValue` 注解可以获取Cookie信息。
- 接收Rest风格参数：Rest风格的特点就是，将参数编写为URL路径的一个部分，更适合获取单个值。
    - `@PathVariable` 用来映射URL中绑定的占位符，通过它可以将URL中占位符的参数绑定到动作类方法的入参中。
    - `@PathVariable` 中的 `id` 必须和 `@RequestMapping` 中的 `{id}` 同名，但不必和方法参数同名。
    - 访问路径中不需要加 `.action`，所以注意一下 `web.xml` 中的核心过滤器是否拦截规则为 `*.action`，如果是，请将拦截规则改为 `/` 再测试这个传参方式。
- 接收键值对参数：`@RequestParam` 更适用于接收键值对类型的参数。
