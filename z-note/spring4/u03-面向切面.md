...
- 开发核心配置文件：不再需要驱动 `@Aspect` 注解：
    - 包扫描：`<context:component-scan base-package="c.j.aop.aspect"/>`
    - 配置AOP区域：`<aop:config></aop:config>`。
    - 配置切点：`<aop:pointcut id="a" expression="PE表达式">`。
    - 配置切面类：`<aop:aspect ref="切面类的<bean>的id">`。
    - 配置通知，使用 `method` 指定通知方法名，使用 `pointcut-ref` 指定切点方法名：
        - `<aop:before />`
        - `<aop:after />`
        - `<aop:after-returning />`，额外使用 `returning` 指定返回值对象。
        - `<aop:after-throwing />`，额外使用 `throwing` 指定异常对象。
        - `<aop:around />`
