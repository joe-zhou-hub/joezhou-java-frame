...
- AOP技术名词：想让A类中的每个方法（共100个）在被调用之前鉴权，被调用之后打印日志，那么：
    - A类就是客户 `Target`，因为聘用代理的就是它。
    - A类里的100个方法，就是100个连接点 `JoinPoint`，若某个方法设置了不使用代理，则不被称为连接点。
    - 这100个连接点，可以组成一个切点 `Pointcut`，切点是一个范围。
    - 方法调用前后进行的处理，叫做前后通知 `Advice`。
    - 方法调用的过程中出现异常时进行的处理，叫做异常通知 `Advice`。
    - 方法return之后执行的处理，叫做返回后通知 `Advice`。
    - 一堆切点和一堆通知组成一个切面类 `Aspect`。
    - 以上整个过程叫做织入 `Weave`。
- PE表达式：Pointcut Expression，作用于切点方法上的 `@Pointcut()` 中，用于指定哪些方法需要使用AOP操作：
    - 模板：`execution([修饰符] 返回类型 [包名].[类名].方法名(形参) [异常])`
    - 修饰符：如 `public`，`public/private` 等，不填写表示友元修饰符。
...
- 开发切面类：
    - 切面类必须由 `@Aspect` 来修饰，且需要被spring容器扫描和管理。
- 开发切点方法：切面类中需要包括一个或多个切点方法，负责指定切点范围：
    - 切点方法上由 `@Pointcut("execution()")` 来配置和指定哪些方法会被执行AOP。
    - 切点方法名随意仅用于区分，方法体必须为空。
- 开发核心配置文件：
    - 包扫描使 `@Component` 生效。
