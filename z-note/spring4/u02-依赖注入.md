...
**源码：** /spring4/
- res: `pom.xml`
- res: `classpath:spring/test/pig.xml`
- src: `c.j.test.PigDao`
- src: `c.j.test.PigService`
- tst: `c.j.test.PigServiceTest.info()`
...
- 开发配置方法：方法上添加 `@Bean` 表示这个方法用于配置一个 `<bean>`：
    ...
    - 方法的返回值就是该bean的最终实例。 
- 开发数据类，注入 `DataSource` 实例并开发 `getConnection()`。
- 开发业务类，注入 `DataSourceDao` 实例并开发 `getConnection()`。
- 测试业务类中的 `getConnection()`。

**源码：** /spring4/
- res: `pom.xml`
- res: `db.properties`
- src: `c.j.java.DataSourceConfig`
- src: `c.j.java.DataSourceDao`
- src: `c.j.java.DataSourceService`
- tst: `c.j.java.DataSourceServiceTest.getConnection()`