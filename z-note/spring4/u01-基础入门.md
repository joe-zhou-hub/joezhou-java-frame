...
- tst: `c.j.start.StartTest.classPathApp()/fileSystemApp()`
...
**源码：** /spring4/
- res: `classpath:spring/start/app-multiple-a/b/c/d.xml`
- src: `c.j.pojo.Student`
...
**源码：** /spring4/
- res: `classpath:spring/start/app-by-id/app-by-name/app-by-class.xml`
- src: `c.j.pojo.Student`
- tst: `c.j.start.StartTest.getBeanById()/getBeanByName()/getBeanByClass()`
...
- tst: `c.j.start.StartTest.buildByConstructor()`
- tst: `c.j.start.StartTest.buildByStaticFactory()`
- tst: `c.j.start.StartTest.buildByDynamicFactory()`

# 6. bean的加载机制

**概念：** `<bean>` 的 `lazy-init` 属性可以调整该bean的加载机制：
- 非延迟加载（默认）：该bean在spring容器初始化时被加载和实例化。
- 延迟加载（懒加载）：该bean在spring容器调用 `getBean()` 时被加载和实例化。

**源码：** /spring4/
- res: `classpath:spring/start/app-lazy.xml`
- src: `c.j.pojo.User`
- tst: `c.j.start.StartTest.lazy()`


**配置：** spring/ioc/app-lazy.xml
```xml
<bean class="com.joe.pojo.User" lazy-init="true"/>
```

```java
@Test
public void lazy() {
    String configLocation = "spring/ioc/app-lazy.xml";
...
**概念：** 
- bean的作用域默认是单例模式，即`scope=singleton`，这导致每次从容器中获取目标bean，返回的都是同一个。
- 如果将bean设置为原型模式，即 `scope="prototype"` ，则每次从容器中获取目标bean，都是重新new出来的一个。
- 单例模式下，bean可以延迟加载也可也非延迟加载。
- 原型模式下，bean必须延迟加载。

**配置：** spring/ioc/app-scope.xml
```xml
<bean class="com.joe.pojo.User" scope="prototype"/>
```
...
@Test
public void scope() {
    String configLocation = "spring/ioc/app-scope.xml";
    ClassPathXmlApplicationContext app = new ClassPathXmlApplicationContext(configLocation);
    User user01 = app.getBean(User.class);
    User user02 = app.getBean(User.class);
...
**概念：** spring调用构造方法的前后，有两个需要自己指定的方法：
- 初始化方法：在创建这个类之后，使用这个类之前调用。
- 销毁方法：在销毁这个类之前调用。
...
**配置：** spring/ioc/app-life.xml
```xml
<bean class="com.joe.pojo.User" init-method="init" destroy-method="destroy"/>
```
...
@Test
public void life() {
    String configLocation = "spring/ioc/app-life.xml";
    ClassPathXmlApplicationContext app = new ClassPathXmlApplicationContext(configLocation);
    app.getBean(User.class);
    app.close();
}
