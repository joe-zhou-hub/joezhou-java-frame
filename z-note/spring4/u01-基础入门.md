...
- 无参构造：spring容器默认调用类的无参构造器进行实例化过程，工作中常用：
    - 只能调用无参构造，无法调用有参构造。
    - 理解：嘿！spring，帮我创建User对象，它拥有无参构造器，你可以直接调用！
- 静态工厂：spring容器可以调用静态工厂的静态方法以完成类的实例化过程：
    - 静态方法中可以手动选择调用类的哪种构造器。
    - 静态工厂中的方法都是静态的，会有线程安全问题。
    - 理解：嘿！spring，帮我调用一下 `UserFactory` 类中的 `getUserByStaticMethod()`，为了效率，这个方法是静态的，所以无需实例化工厂，直接调用即可。
- 实例工厂：spring容器可以调用实例工厂的实例方法以完成类的实例化过程：
    - 实例方法中可以手动选择调用类的哪种构造器。
    - 实例工厂中的方法都是非静态的，不会有线程安全问题。
    - 理解：嘿！spring，帮我调用一下 `UserFactory` 类中的 `getUserByDynamicMethod()`，为了线程安全，这个方法不是静态的，所以你得先实例化工厂，然后才能调用这个方法。

**源码：** /spring4/
- res: `classpath:spring/start/app-by-constructor.xml`
- res: `classpath:spring/start/app-by-static-factory.xml`
- res: `classpath:spring/start/app-by-dynamic-factory.xml`
- src: `c.j.pojo.User`
- src: `c.j.factory.UserFactory`
- tst: `c.j.start.StartTest.buildBeanByConstructor()`
- tst: `c.j.start.StartTest.buildBeanByStaticFactory()`
- tst: `c.j.start.StartTest.buildBeanByDynamicFactory()`